...
# ── CONFIGURATION ───────────────────────────────────────────────
# All secrets are set via environment variables in the PythonAnywhere WSGI file.
# Never hardcode real values here — this file is stored in version control.
...
# Path to service account key file on the server (not in the repo)
GOOGLE_KEY_FILE = os.environ.get("GOOGLE_KEY_FILE", "/home/vaishnavisupy/mysite/google_keys.json")
...
def log_to_sheets(d, branches, submitted_at):
    try:
        ...
        if not os.path.exists(GOOGLE_KEY_FILE):
            logger.error(f"[Sheets] Key file not found at {GOOGLE_KEY_FILE}")
            return
...
def get_gmail_access_token():
    if not GMAIL_REFRESH_TOKEN:
        logger.warning("[Gmail] GMAIL_REFRESH_TOKEN not set.")
        return None
...
def send_email_notification(d, branches, submitted_at):
    ...
    if not token:
        logger.warning("[Email] Gmail token unavailable — skipping.")
        return
...
<body style="font-family:Arial,sans-serif;background:#f4f1fa;margin:0;padding:24px">
<div style="max-width:680px;margin:0 auto">
  ...
  <div style="background:#fff;padding:28px 32px;border-radius:0 0 12px 12px;box-shadow:0 4px 16px rgba(80,51,144,.1)">
    ...
    <table style="width:100%;border-collapse:collapse;margin-bottom:20px;background:#f9f7fe;border-radius:8px">
      {row("Name", d.get("champion_name",""))}{row("Title", d.get("champion_title",""))}{row("Email", champ_email)}{row("Phone", d.get("champion_phone",""))}
    </table>
    <h3 style="color:#321e57;font-size:13px;border-left:3px solid #f59e0b;padding-left:10px;margin:0 0 10px">Finance POC</h3>
    <table style="width:100%;border-collapse:collapse;margin-bottom:20px;background:#fffbf0;border-radius:8px">
      {row("External Firm?", d.get("accounting_external",""))}{row("Name", d.get("finance_name",""))}{row("Title", d.get("finance_title",""))}{row("Email", d.get("finance_email",""))}{row("Phone", d.get("finance_phone",""))}
    </table>
    <h3 style="color:#321e57;font-size:13px;border-left:3px solid #3b82f6;padding-left:10px;margin:0 0 10px">IT Contact</h3>
    <table style="width:100%;border-collapse:collapse;margin-bottom:20px;background:#f0f7ff;border-radius:8px">
      {row("Same as Champion?", d.get("it_same_as_champion",""))}{row("Name", d.get("it_name",""))}{row("Email", d.get("it_email",""))}{row("POS System", d.get("pos_system",""))}{row("Accounting SW", d.get("accounting_software",""))}
    </table>
    ...
    <table style="width:100%;border-collapse:collapse;margin-bottom:20px;font-size:12px">
      <tr style="background:#321e57;color:#fff">
        <th style="padding:8px 10px;text-align:left">#</th><th style="padding:8px 10px;text-align:left">Name</th>
        <th style="padding:8px 10px;text-align:left">Address</th><th style="padding:8px 10px;text-align:left">Cost Center</th>
    ...
    <table style="width:100%;border-collapse:collapse;margin-bottom:20px;background:#f9f7fe;border-radius:8px">
      {row("Order Method", d.get("ordering_method",""))}{row("PO Approver", d.get("po_approver",""))}{row("Ordering Structure", d.get("ordering_structure",""))}
      {row("Stock Counts", d.get("stock_counts",""))}{row("Inventory System", d.get("inventory_system",""))}
      {row("Current Food Cost %", d.get("food_cost_current",""))}{row("Target Food Cost %", d.get("food_cost_target",""))}
      {row("COGS Method", d.get("cogs_method",""))}{row("Invoice Delivery", d.get("invoice_delivery",""))}
    </table>
    ...
    <table style="width:100%;border-collapse:collapse;margin-bottom:20px;background:#fff5f5;border-radius:8px">
      {row("Top Problem", d.get("top_problem",""))}{row("CSM Notes", d.get("extra_notes",""))}{row("Blockers", d.get("blockers",""))}{row("Target Go-Live", d.get("golive_date",""))}
    </table>
    <h3 style="color:#321e57;font-size:13px;border-left:3px solid #06b6d4;padding-left:10px;margin:0 0 10px">File Links</h3>
    <table style="width:100%;border-collapse:collapse;margin-bottom:20px;background:#f0fdff;border-radius:8px">
      {row("Invoices", d.get("invoices_link",""))}{row("Supplier Details", d.get("suppliers_link",""))}
    </table>
  </div>
  <p style="text-align:center;font-size:10px;color:#aaa;margin-top:16px">supy.io &middot; Onboarding Automation</p>
</div>
</body></html>"""
    ...
    if send_r.status_code == 200:
        ...
        logger.error(f"[Email Error]: {send_r.status_code} {send_r.text}")
    ...
    try:
        payload = {
            "blocks": [
                ...
                {"type": "context", "elements": [{"type": "mrkdwn", "text": "Supy Onboarding Form · supy.io"}]}
            ]
        }
...
def get_hubspot_token():
    if not CLIENT_ID or not CLIENT_SECRET or not REFRESH_TOKEN:
        logger.warning("[HubSpot] Credentials not set.")
        return None
...
def webhook():
    ...
    if d.get("branches_json"):
        try:
            branches = json.loads(d["branches_json"])
        except Exception as e:
    ...
    logger.warning(f"Note Error: {note_r.text}")
